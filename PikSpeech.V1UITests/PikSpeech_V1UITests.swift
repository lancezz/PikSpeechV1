...
class PikSpeech_V1UITests: XCTestCase {
    ...
    func testnavigation() {
        ...
        let confirmButton = app.alerts["Parental Control"].buttons["Confirm"]
        confirmButton.tap()
        settingsbuttonButton.tap()
        confirmButton.tap()
        app.collectionViews.cells.otherElements.containing(.staticText, identifier:"cat").element.tap()
        app.children(matching: .window).element(boundBy: 0).children(matching: .other).element.children(matching: .other).element.children(matching: .collectionView).element(boundBy: 3).cells.otherElements.containing(.staticText, identifier:"chicken").element.tap()
        
        let speakbuttonButton = app.buttons["SpeakButton"]
        speakbuttonButton.tap()
        speakbuttonButton.tap()
